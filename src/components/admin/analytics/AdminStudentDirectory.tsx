...
export default function AdminStudentDirectory({
  ...
  const filtered = useMemo(() => {
    ...
    return students.filter(({ summary, email }) => {
      ...
      if (clinicFilter !== 'all') {
        if (!summary.clinic_names?.includes(clinicFilter)) return false;
      }
      return true;
    });
          ...
          <Table className="min-w-[900px]">
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-xs text-center">Eval</TableHead>
                ...
                filtered.map((student) => {
                  ...
                  return (
                    <TableRow key={summary.id} className="hover:bg-muted/30">
                      ...
                      <TableCell>
                        <AttentionBadge level={summary.attention_level} />
                      </TableCell>
                      ...
                      <TableCell className="text-xs text-center tabular-nums">
                        {summary.avg_evaluation_score != null
                          ? summary.avg_evaluation_score.toFixed(1)
                          : '—'}
                      </TableCell>
                      <TableCell className="text-xs text-muted-foreground whitespace-nowrap">
                        {new Date(summary.joined_at).toLocaleDateString('en-US', {
                          month: 'short',
                          day: 'numeric',
                          year: '2-digit',
                        })}
