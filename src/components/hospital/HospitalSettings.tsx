...
export default function HospitalSettings() {
  ...
  const isGmailConnected = Boolean(hospitalPage.gmail_email);

  async function handleConnectGmail() {
    if (!hospitalPage) return;
    ...
    try {
      ...
      // Store the return path so the callback page can send the admin back here.
  ...
  async function handleDisconnectGmail() {
    ...
    try {
      ...
      toast({ title: 'Gmail disconnected', description: 'Emails will now be sent via ClinicalHours.' });
      refetch();
    } catch (err: unknown) {
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Connect your Gmail account to send applicant emails directly from your own address.
            If not connected, emails are sent via ClinicalHours.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {isGmailConnected ? (
            <div className="flex items-center justify-between gap-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  {/* Gmail "G" colour dot */}
                  <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-white border border-border shadow-sm flex-shrink-0">
                  ...
                  <span className="text-sm font-medium">{hospitalPage.gmail_email}</span>
                </div>
                {hospitalPage.gmail_connected_at && (
                  <p className="text-xs text-muted-foreground pl-8">
                    Connected{' '}
                    {new Date(hospitalPage.gmail_connected_at).toLocaleDateString(undefined, {
                      year: 'numeric',
                      month: 'short',
                      day: 'numeric',
                    })}
