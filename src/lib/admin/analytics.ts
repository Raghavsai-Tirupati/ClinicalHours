...
// Aliases for /analytics hub components
export type AdminDashboardKPIs = AdminKpis;
export type AdminStudentSummary = StudentSummary;
export type UnifiedActivityRow = UnifiedActivity;
...
export async function fetchAdminKPIs(
  ...
  return fetchAdminKpis(since.toISOString(), until.toISOString(), clinicId);
}
...
export async function fetchAdminStudentSummaries(): Promise<StudentSummary[]> {
  return fetchStudentSummaries(5000);
}
