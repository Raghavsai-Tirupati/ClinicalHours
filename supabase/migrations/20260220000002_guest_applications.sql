...
-- • Makes student_id nullable so unauthenticated visitors can apply
-- • Adds applicant_email / applicant_name columns for guest identity
-- • Replaces broken UNIQUE(account_id, student_id) with partial indexes
-- • Adds submit_guest_hospital_application() SECURITY DEFINER RPC
-- ─────────────────────────────────────────────────────────────────────────────
...
-- 3. Drop the old table-level UNIQUE constraint (NULL != NULL breaks it for guests)
ALTER TABLE public.hospital_applications
...
--    Bypasses RLS so anonymous visitors can submit without an account.
CREATE OR REPLACE FUNCTION public.submit_guest_hospital_application(
  p_account_id uuid,
  p_name       text,
  p_email      text,
  p_answers    jsonb   -- array of {question_id, answer_text, answer_options}
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Duplicate guest application check
  IF EXISTS (
  ...
  -- Insert answers (skip if empty array)
